...
static bool Dice_Critical_Success(
    int effectiveSkill,
    int diceRoll) {
    ...
    if (effectiveSkill > 20) {

        effectiveSkill = 20;
    }

    if (effectiveSkill > 14) {

        crit += (effectiveSkill - 14);
    }
...
static bool Dice_Critical_Failure(
    int effectiveSkill,
    int diceRoll) {
    ...
    }else if ((diceRoll == 17) && (effectiveSkill < 16)) {

        ret = true;

    }else if (diceRoll >= effectiveSkill + 10) {
